...
class User:
  ...
  def add(self):
    ...
    #ensure that the company exists (_id is ObjectId)
    id_as_str = str(expected_data['companyID'])
    company = app.db.companies.find_one({"_id": app.ObjectId(id_as_str)})
    #print company
    print("company", company)
    if not company:
      return tools.JsonResp({"message": "Company does not exist"}, 400)
